...
recognizer = models.mobilenet_v2(pretrained=True)
...
Face_Images = os.path.join(os.getcwd(), "Face_Images\ParkSeungJae") #이미지 폴더 지정
print (Face_Images)
...
for root, dirs, files in os.walk(Face_Images) : #파일 목록 가져오기
    for file in files :
        if file.endswith("jpeg") or file.endswith("jpg") or file.endswith("png") : #이미지 파일 필터링
            ...
            faces = face_cascade.detectMultiScale(gray_image, scaleFactor = 1.5, minNeighbors = 5)#얼굴 찾기
